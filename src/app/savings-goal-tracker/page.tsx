...
export default function SavingsGoalTrackerPage() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        setTargetAmount(parsed.targetAmount || 0);
        setCurrentSaved(parsed.currentSaved || 0);
      } catch {
        console.error("Failed to load saved goal");
      }
    }
  }, []);
  ...
  useEffect(() => {
    const data = JSON.stringify({ goalName, targetAmount, currentSaved });
    localStorage.setItem("savings-goal", data);
