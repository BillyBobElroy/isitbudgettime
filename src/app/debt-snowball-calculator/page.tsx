...
export default function DebtSnowballCalculatorPage() {
  ...
  useEffect(() => {
    const raw = localStorage.getItem("snowball-debts");
    if (raw) {
      try {
        ...
        console.error("Failed to parse saved debts");
      }
    }
  }, []);

  // Save to localStorage on debts change
  useEffect(() => {
  ...
  const clearDebts = () => {
    if (confirm("Clear all saved debts?")) {
      localStorage.removeItem("snowball-debts");
      setDebts([{ name: "", balance: 0, minPayment: 0, interestRate: 0 }]);
    }
  };
    ...
    <>
      <main className="min-h-screen p-8 bg-gray-50 flex flex-col items-center">
              ...
              {sortedDebts.map((debt, idx) => (
                <li key={idx} className="border p-4 rounded">
                  ...
                  <p className="mt-1 text-sm text-gray-600">
                    ...
                    {estimatedMonths(debt) === Infinity ? "-" : estimatedMonths(debt)}
                  </p>
                </li>
              ))}
            </ul>
        ...
        <section className="max-w-3xl text-gray-700 text-sm">
          ...
          <p className="mb-2">
            The debt snowball method focuses on paying off debts from the smallest balance to the largest.
            You gain momentum and motivation with every debt you eliminate!
          </p>
